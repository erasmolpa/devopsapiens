...
Cuando nos toca arrancar un proyecto en Operaciones/It ,en el que se pretende usar soluciones enfocadas a IaC , es demasiado fácil y común, llevar la conversación al contexto de la solución. En muchos casos,nos posturamos hacia una solución por experiencias previas, comodidad o afinidad hacia una tecnología/entorno/lenguaje. 

Esto es totalmente lógico y normal. Nos gusta sentirnos cómodos y seguros en el día a día y no hay nada malo en ello. Pero la realidad es que , llevar a buen puerto la IaC es algo bastante complejo , y hay infinidad de cosas a tener en cuenta , asi que , seamos de mente abierta y asumamos que igual en algún punto o paso del proceso , nos tocará seguir usando ese script bash que tanto odias. Sand but true. 


Este post, algo atípico para pretender ser el primero de una serie, vamos a mencinar algunos principios y características que creo que pueden ser útiles en este tipo de toma de decisiones.En futuros post , iremos más en profundidad con muchos cosas por las que vamos a pasar de puntillas en este post. 
...
Por mucho que puedas decir eso de; "en mi local funciona", seguro que no estás exento de alguna que otra batalla contra la máquina.

Si además dependes de otras personas/equipos para disponer de tan preciados recursos, probablemente también te ha tocado esperar, quizás días,o incluso semanas, para tener un entorno. Y para hacerlo más divertido, es probable que te haya tocado formalizar la petición, creando un ticket en la herramienta de turno, o mejor aún, enviando un email.

Y todo este trabajo "extra" y esfuerzo, de manera recurrente, se hace duro de llevar.

Todo esto cuesta mucho dinero a las compañías.Como también cuesta dinero tener infraestructura creada sin ser usada , solo porque alguien la pedirá en algún momento y es más cara borrarla y volver a crearla, que mantenerla viva. (sad but true)

El paradigma de La infraestructura esta evolucionando continuamente, haciendo que los problemas conocidos , como los mencionados anteriormente, tengan solución, pero ese continua evolución esta haciendo también que la gestión de procesos e infraestructura sea cada vez más desafiante y "divertida" :) 

Esto también impacta directamente al dinamismo de la propia compañía, a como plantean sus estrategias técnicas y todo esto esta haciendo que la IaC sea un factor clave a cosiderar.

# La Solución:

_"IaC es un enfoque que permite crear y configurar recursos de infraestructura de manera programática"_

Parece que en algo estamos todos deacuerdo; lo de crear infraestructura con la UI del proveedor de cloud de turno, o usando un script que tienes en tu máquina, esta bien para una POC,un entorno de pruebas...pero para poco más.tiene cierto valor en ese momento/contexto.

IaC nos permite aplicar los principios de desarrollo de software a la gestión de la infraestructura. Esto significa que podemos aprovechar las prácticas de desarrollo ágil, como la integración continua y la entrega continua, para iterar rápidamente en nuestra infraestructura y responder de manera eficiente a los cambios en los requisitos del negocio.

La IaC, es para muchos de nosotros, sinónimo de oportunidad y desafío. dImplantar Iac en una compañía , es todo un desafío, pero dentro del desafío que supone, es demasiado gratificante ver como se mejora la vida de las personas y los procesos de la compañia
...
Estos son los principios que se me vienen a la cabeza y que he ido recolectando como tales , de libros , post etc..Algunos son intrinsecos de toda herramienta de IaC , algunos están más de nuestras manos y buen hacer. Ejemplo , toda herramienta de IaC es idempotente por definicion,toda herramienta de IaC esta diseñada para generar , de mejor o peor manera, codigo reusable.  
...
_es que te contaron sobre DevOps, ahora es posible_

Que trillada esta est
...
- Planteate los peores escenarios.

- Plateate también los mejores escenearios. 
